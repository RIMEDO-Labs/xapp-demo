...
import (
	...
	"github.com/RIMEDO-Labs/xapp-demo/pkg/monitoring"
	"github.com/RIMEDO-Labs/xapp-demo/pkg/rnib"
	"google.golang.org/protobuf/proto"
)

var log = logging.GetLogger("demo-xapp", "e2", "manager")
...
func NewManager(options Options, indCh chan *demo.E2NodeIndication) (Manager, error) {
...
type Manager struct {
	...
	indCh       chan *demo.E2NodeIndication
...
func (m *Manager) watchE2Connections(ctx context.Context) error {
	...
	for topoEvent := range ch {
		if topoEvent.Type == topoapi.EventType_ADDED || topoEvent.Type == topoapi.EventType_NONE {
			...
			log.Info("Information about ", e2NodeID)
...
func (m *Manager) createSubscription(ctx context.Context, e2nodeID topoapi.ID, triggerType e2sm_mho.MhoTriggerType) error {
	...
	subName := fmt.Sprintf("xapp-demo-subscription-%s", triggerType)
